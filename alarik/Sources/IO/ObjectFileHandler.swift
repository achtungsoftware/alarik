...
struct ObjectFileHandler {
    ...
    /// into a filesystem path. Every legitimate version id is generated by
    /// `ObjectMeta.generateVersionId()` (32 lowercase hex chars) or the literal `"null"` sentinel -
    /// neither ever contains a separator or `..`, so stripping them can't break a real lookup; a
    /// versionId that needed stripping simply resolves to a nonexistent file (NoSuchVersion), the
    /// correct S3 outcome. Without this, `?versionId=../../other-bucket/key` on GET/DELETE escaped
    /// the key's `.versions/` directory - a cross-bucket read/delete for any authenticated client.
